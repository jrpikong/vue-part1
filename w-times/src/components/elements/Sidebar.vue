<template>
    <aside>
        <div class="widget widget-bg">
            <div class="heading">
                <h2 class="main-heading">About Author</h2>
                <span class="heading-ping"></span>
            </div>
            <div class="author-widget">
                <div class="auth-pic">
                    <img src="../../../static/images/users/author2.jpg" class="img-responsive" alt="">
                </div>
                <div class="auth-meta">
                    <h4> Emily Copper</h4>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim minim veniam, quis nostrud exercitation .</p>
                    <div class="social-media-icons">
                        <ul>
                            <li> <a href=""><i class="ti-facebook"></i></a></li>
                            <li> <a href=""><i class="ti-twitter"></i></a></li>
                            <li> <a href=""><i class="ti-instagram"></i></a></li>
                            <li> <a href=""><i class="ti-linkedin"></i></a></li>
                            <li> <a href=""><i class="ti-google"></i></a></li>
                            <li> <a href=""><i class="ti-pinterest"></i></a></li>
                        </ul>
                    </div>
                    <a href="" class="btn btn-colored-blog"> Read More</a>
                </div>
            </div>
        </div>
        <div class="widget widget-bg">
            <div class="heading">
                <h2 class="main-heading">Connact Now</h2>
                <span class="heading-ping"></span>
            </div>
            <div class="social-counter-widget">
                <ul class="social-counter-list-2 social-style-3">
                    <li class="social-item">
                        <a href="">
                            <div class="social-item-icon twitter">
                                <i class="fa fa-twitter"></i> <span class="title">Twitter</span>
                                <div class="social-item-meta">
                                    <span class="social-item-num">573</span>
                                </div>
                            </div>
                        </a>
                    </li>
                    <li class="social-item">
                        <a href="">
                            <div class="social-item-icon facebook">
                                <i class="fa fa-facebook"></i> <span class="title">Facebook</span>
                                <div class="social-item-meta">
                                    <span class="social-item-num">15732</span>
                                </div>
                            </div>
                        </a>
                    </li>
                    <li class="social-item">
                        <a href="">
                            <div class="social-item-icon youtube">
                                <i class="fa fa-youtube"></i> <span class="title">Youtube</span>
                                <div class="social-item-meta">
                                    <span class="social-item-num">136845</span>
                                </div>
                            </div>
                        </a>
                    </li>
                    <li class="social-item">
                        <a href="">
                            <div class="social-item-icon instgram">
                                <i class="fa fa-instagram"></i><span class="title">Instagram</span>
                                <div class="social-item-meta">
                                    <span class="social-item-num">65489</span>
                                </div>
                            </div>
                        </a>
                    </li>
                    <li class="social-item">
                        <a href="">
                            <div class="social-item-icon gplus">
                                <i class="fa fa-google-plus"></i><span class="title">Google+</span>
                                <div class="social-item-meta">
                                    <span class="social-item-num">7233</span>
                                </div>
                            </div>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
        <div class="widget widget-bg">
            <div class="heading">
                <h2 class="main-heading">Advertizement</h2>
                <span class="heading-ping"></span>
            </div>
            <div class="ad-div style-box">
                <a href="https://themeforest.net/item/hashpro-listing-and-directory-template/16901616">
                    <img src="../../../static/images/ads/300-8.jpg" class="img-responsive " alt="">
                </a>
            </div>
        </div>
        <div class="widget widget-bg">
            <div class="tabs">
                <div role="tabpanel">
                    <!-- Nav tabs -->
                    <ul class="nav nav-tabs nav-justified" role="tablist">
                        <li class="active" role="presentation"> <a aria-controls="popular" aria-expanded="true" data-toggle="tab"
                                                                   href="#popular" role="tab">Popular</a> </li>
                        <li class="" role="presentation"> <a aria-controls="comments-count-count" aria-expanded="false"
                                                             data-toggle="tab" href="#comments-count-count" role=
                                                                     "tab">recent</a> </li>
                    </ul>
                    <div class="tab-content">
                        <div class="tab-pane active" id="popular" role="tabpanel">
                            <ul class="tabs-posts">
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-1.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">Get a stern talking-to from your parents with Kit Kat's Lunar</a> </div>
                                    <ul class="post-tools">
                                        <li>  January 21, 2016 </li>
                                        <li title="Comments"> <i class="ti-thought"></i> 953 </li>
                                    </ul>
                                </li>
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-2.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">The assault on federally supported </a> </div>
                                    <ul class="post-tools">
                                        <li>  February 28, 2016 </li>
                                        <li title="Comments"> <i class="ti-thought"></i> 105 </li>
                                    </ul>
                                </li>
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-3.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">Presidential campaigns is too odd</a> </div>
                                    <ul class="post-tools">
                                        <li>  March 01, 2016 </li>
                                        <li title="Comments"> <i class="ti-thought"></i> 305 </li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                        <div class="tab-pane" id="comments-count-count" role="tabpanel">
                            <ul class="tabs-posts">
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-2.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">The assault on federally supported </a> </div>
                                    <ul class="post-tools">
                                        <li title="Comments"> <i class="ti-thought"></i> 105 </li>
                                    </ul>
                                </li>
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-5.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">Taco Bell's newest creation may be the grossest fast food invention yet</a> </div>
                                    <ul class="post-tools">
                                        <li title="Comments"><a href=""> <i class="ti-thought"></i> 35</a> </li>
                                    </ul>
                                </li>
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-4.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">MBD just hired a big shot to run its virtual reality division</a> </div>
                                    <ul class="post-tools">
                                        <li title="Comments"> <a href=""><i class="ti-thought"></i> 163</a> </li>
                                    </ul>
                                </li>
                                <li>
                                    <div class="pic"> <a href="standard-post.html"><img alt="" class="img-responsive" src="../../../static/images/small-posts/small-1.jpg"></a> </div>
                                    <div class="caption"> <a href="standard-post.html">Get a stern talking-to from your parents with Kit Kat's Lunar</a> </div>
                                    <ul class="post-tools">
                                        <li title="Comments"> <a href=""><i class="ti-thought"></i> 953</a> </li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="widget widget-bg">
            <div class="heading">
                <h2 class="main-heading">Archive Posts</h2>
                <span class="heading-ping"></span>
            </div>
            <ul class="tabs-posts">
                <li>
                    <div class="caption"> <a href="standard-post.html">In their own words: What 'Invasion Day' means to a rural Indigenous</a> </div>
                </li>
                <li>
                    <div class="caption"> <a href="standard-post.html">Presidential campaigns is too odd</a> </div>
                </li>
                <li>
                    <div class="caption"> <a href="standard-post.html">PWhy people 'breadcrumb' each other on Tinder </a> </div>
                </li>
                <li>
                    <div class="caption"> <a href="standard-post.html">Don't believe the gruesome rumors about this MLB pitcher's death</a> </div>
                </li>
            </ul>
        </div>
        <div class="widget widget-bg ">
            <div class="heading">
                <h2 class="main-heading">Slider Post Widget</h2>
                <span class="heading-ping"></span>
            </div>
            <div class="featured-post-slider-single-post owl-carousel owl-theme">
                <div class="item">
                    <div class="latest-news-grid grid-1">
                        <div class="picture">
                            <div class="category-image">
                                <a href="standard-post.html">
                                    <img alt="" class="img-responsive" src="../../../static/images/posts/19.jpg">
                                </a>
                                <div class="catname">
                                    <a class="btn btn-maroon" href="#">
                                        <div>Sports</div>
                                    </a>
                                </div>
                                <div class="hover-show-div">
                                    <a href="" class="post-type">
                                        <i class="ti-music-alt"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="detail">
                            <div class="caption">
                                <h5>
                                    <a href="standard-post.html">Mysterious Footbal Player steps forward to lead the National Park resistance</a>
                                </h5>
                            </div>
                            <ul class="post-tools">
                                <li> by <a href=""> <strong> Arsoo</strong> </a></li>
                                <li>  March 10, 2017 </li>
                                <li> <a href=""><i class="ti-thought"></i> 57 </a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <div class="latest-news-grid grid-1">
                        <div class="picture">
                            <div class="category-image">
                                <a href="standard-post.html">
                                    <img alt="" class="img-responsive" src="../../../static/images/posts/15.jpg">
                                </a>
                                <div class="catname">
                                    <a class="btn btn-green" href="#">
                                        <div>Travel</div>
                                    </a>
                                </div>
                                <div class="hover-show-div">
                                    <a href="" class="post-type">
                                        <i class="ti-music-alt"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="detail">
                            <div class="caption">
                                <h5>
                                    <a href="standard-post.html">Computer issue forces ground-stop for United Airlines in the U.S.</a>
                                </h5>
                            </div>
                            <ul class="post-tools">
                                <li> by <a href=""> <strong> Arsoo</strong> </a></li>
                                <li>  March 10, 2017 </li>
                                <li> <a href=""><i class="ti-thought"></i> 57 </a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <div class="latest-news-grid grid-1">
                        <div class="picture">
                            <div class="category-image">
                                <a href="standard-post.html">
                                    <img alt="" class="img-responsive" src="../../../static/images/posts/19.jpg">
                                </a>
                                <div class="catname">
                                    <a class="btn btn-maroon" href="#">
                                        <div>Sports</div>
                                    </a>
                                </div>
                                <div class="hover-show-div">
                                    <a href="" class="post-type">
                                        <i class="ti-music-alt"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="detail">
                            <div class="caption">
                                <h5>
                                    <a href="standard-post.html">Mysterious Footbal Player steps forward to lead the National Park resistance</a>
                                </h5>
                            </div>
                            <ul class="post-tools">
                                <li> by <a href=""> <strong> Arsoo</strong> </a></li>
                                <li>  March 10, 2017 </li>
                                <li> <a href=""><i class="ti-thought"></i> 57 </a></li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="item">
                    <div class="latest-news-grid grid-1">
                        <div class="picture">
                            <div class="category-image">
                                <a href="standard-post.html">
                                    <img alt="" class="img-responsive" src="../../../static/images/posts/17.jpg">
                                </a>
                                <div class="catname">
                                    <a class="btn btn-lightblue" href="#">
                                        <div>technology</div>
                                    </a>
                                </div>
                                <div class="hover-show-div">
                                    <a href="" class="post-type">
                                        <i class="ti-image"></i>
                                    </a>
                                </div>
                            </div>
                        </div>
                        <div class="detail">
                            <div class="caption">
                                <h5>
                                    <a href="standard-post.html">Mark Zuckerberg's charity is buying a search engine for research papers</a>
                                </h5>
                            </div>
                            <ul class="post-tools">
                                <li> by <a href=""> <strong> Arsoo</strong> </a></li>
                                <li>  March 10, 2017 </li>
                                <li> <a href=""><i class="ti-thought"></i> 57 </a></li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </aside>
</template>
